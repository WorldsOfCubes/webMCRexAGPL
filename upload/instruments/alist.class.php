...
class ConfigManager {
	...
	public static function SaveMainConfig() {
		...
		if (file_put_contents(MCR_ROOT.'main.cfg.php', $txt) === false)
			return false;

		return true;
	}

	public static function SaveDonateConfig() {
		global $donate;

		$txt = '<?php'.PHP_EOL;
		$txt .= '$donate = '.var_export($donate, true).';'.PHP_EOL;
		$txt .= '/* This file was auto-generated by '.PROGNAME.' */'.PHP_EOL;
		$txt .= '?>';

		if (file_put_contents(MCR_ROOT.'donate.cfg.php', $txt) === false)
			return false;
...
class ThemeManager extends View {
	...
	/** @const */
	public static $false_dir = array('.', '..', '.htaccess',);

	/** @const */
	public static $true_info = array('id', 'name', 'version', 'author', 'about', 'editable', 'work_with',);
	...
	public static function GetThemeInfo($theme_id, $theme_info_txt = false) {
		...
		if (empty($theme_info['id'])) {
			...
			if (!empty($theme_info['name']))
				$theme_info['id'] = self::GenerateTIDbyName($theme_info['name']); else return false;
		}
...
class ControlManager extends View {
	...
	public function ShowUserListing($list = 1, $search_by = 'name', $input = false) {
		...
		if ($search_by == 'name')
			$result = $db->execute("SELECT `{$bd_users['id']}` FROM `{$bd_names['users']}` WHERE {$bd_users['login']} LIKE '%$input%' ORDER BY {$bd_users['login']} LIMIT ".(10 * ($list - 1)).",10"); elseif ($search_by == 'none')
			$result = $db->execute("SELECT `{$bd_users['id']}` FROM `{$bd_names['users']}` ORDER BY {$bd_users['login']} LIMIT ".(10 * ($list - 1)).",10");
		elseif ($search_by == 'ip')
			$result = $db->execute("SELECT `{$bd_users['id']}` FROM `{$bd_names['users']}` WHERE {$bd_users['ip']} LIKE '%$input%' ORDER BY {$bd_users['login']} LIMIT ".(10 * ($list - 1)).",10");
		elseif ($search_by == 'lvl') {

			$result = $db->execute("SELECT `id` FROM `{$bd_names['groups']}` WHERE `lvl`='$input'");

			$id_group = $db->fetch_array($result, MYSQL_NUM);
			$input = $id_group[0];

			$result = $db->execute("SELECT `{$bd_users['id']}` FROM `{$bd_names['users']}` WHERE `{$bd_users['group']}` = '$input' ORDER BY {$bd_users['login']} LIMIT ".(10 * ($list - 1)).",10");
		}
		...
		if ($search_by == 'name')
			$result = $db->execute("SELECT COUNT(*) FROM `{$bd_names['users']}` WHERE {$bd_users['login']} LIKE '%$input%'"); elseif ($search_by == 'none')
			$result = $db->execute("SELECT COUNT(*) FROM `{$bd_names['users']}`");
		elseif ($search_by == 'ip')
			$result = $db->execute("SELECT COUNT(*) FROM `{$bd_names['users']}` WHERE {$bd_users['ip']} LIKE '%$input%'");
		elseif ($search_by == 'lvl')
			$result = $db->execute("SELECT COUNT(*) FROM `{$bd_names['users']}` WHERE `{$bd_users['group']}`='$input'");
	...
	public function ShowServers($list) {
		...
		$result = $db->execute("SELECT * FROM `{$bd_names['servers']}` ORDER BY priority DESC LIMIT ".(10 * ($list - 1)).",10");
		$resnum = $db->num_rows($result);

		if (!$resnum) {
			include $this->GetView('admin/server/servers_not_found.html');
			return ob_get_clean();
		}
		...
		$result = $db->execute("SELECT COUNT(*) FROM `{$bd_names['servers']}`");
		$line = $db->fetch_array($result);
		$resnum = $line[0];
	...
	public function ShowIpBans($list) {
		...
		$result = $db->execute("SELECT * FROM `{$bd_names['ip_banning']}` ORDER BY ban_until DESC LIMIT ".(10 * ($list - 1)).",10");
		$resnum = $db->num_rows($result);
		...
		$result = $db->execute("SELECT COUNT(*) FROM `{$bd_names['ip_banning']}`");
		$line = $db->fetch_array($result);
		$resnum = $line[0];
