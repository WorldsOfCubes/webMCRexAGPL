...
class ConfigManager {
	...
	$txt .= '?>';
		...
		if (file_put_contents(MCR_ROOT.'main.cfg.php', $txt) === false) return false;
	...
	public static function SaveDonateConfig()
	{
		...
		if (file_put_contents(MCR_ROOT . 'donate.cfg.php', $txt) === false) return false;
...
private $work_skript;
	...
	global $db, $bd_users,$bd_names;
	    ...
	    if ($search_by == 'name') $result = $db->execute("SELECT `{$bd_users['id']}` FROM `{$bd_names['users']}` WHERE {$bd_users['login']} LIKE '%$input%' ORDER BY {$bd_users['login']} LIMIT ".(10*($list-1)).",10");
    elseif ($search_by == 'none') $result = $db->execute("SELECT `{$bd_users['id']}` FROM `{$bd_names['users']}` ORDER BY {$bd_users['login']} LIMIT ".(10*($list-1)).",10");
	elseif ($search_by == 'ip'  ) $result = $db->execute("SELECT `{$bd_users['id']}` FROM `{$bd_names['users']}` WHERE {$bd_users['ip']} LIKE '%$input%' ORDER BY {$bd_users['login']} LIMIT ".(10*($list-1)).",10");
	elseif ($search_by == 'lvl' ) {
	    ...
	    if ($search_by == 'name') $result = $db->execute("SELECT COUNT(*) FROM `{$bd_names['users']}` WHERE {$bd_users['login']} LIKE '%$input%'");
	elseif ($search_by == 'none') $result = $db->execute("SELECT COUNT(*) FROM `{$bd_names['users']}`");
	elseif ($search_by == 'ip'  ) $result = $db->execute("SELECT COUNT(*) FROM `{$bd_names['users']}` WHERE {$bd_users['ip']} LIKE '%$input%'");
	elseif ($search_by == 'lvl' ) $result = $db->execute("SELECT COUNT(*) FROM `{$bd_names['users']}` WHERE `{$bd_users['group']}`='$input'");
