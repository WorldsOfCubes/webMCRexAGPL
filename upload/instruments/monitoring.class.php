...
Class Server extends Item {
	...
	public function SetText($var, $field = 'name') {
		global $db;
		if (!$this->Exist())
			return false; else if (!$field == 'name' and !$field == 'info')
			return false;
		...
		if ($field == 'name')
			$this->name = $var; else  $this->info = $var;
	}
	...
	public function UpdateState($extra = false) {
		...
		if ($users_list) {
			...
			if ($numpl == 1)
				$system_users = $users_list[0]; else {

				for ($i = 0; $i < $numpl; $i++) {
					if ($i == 0)
						$system_users .= $users_list[$i]; else            $system_users .= ','.$users_list[$i];
				}
			}
		}
		...
		if (!empty($full_state))    // name='".$full_state['hostname']."'
			$db->execute("UPDATE `".$this->db."` SET numpl='".$db->safe($numpl)."',slots='".$db->safe($full_state['maxplayers'])."',players='".$db->safe($system_users)."',online='1' WHERE id='".$this->id."'"); else
	...
	public function GetPlayers() {
		...
		if (!sizeof(explode(',', $list)) and !$numpl)
			return array("Сервер пуст", 0); else  return array($list, $numpl);
	}
	...
	public function ShowHolder($type = 'side', $server_prefix = '') {
		...
		if ((int)$this->slots != -1)
			$server_pl_inf = $this->numpl.'/'.$this->slots; else
		...
		switch ($type) {
			...
			case 'game':
				...
				if ($this->online)
					include $this->GetView('serverstate_'.$type.'_online.html'); else include $this->GetView('serverstate_'.$type.'_offline.html');
	...
	public function getInfo() {
		...
		return array('id' => $this->id, 'address' => $this->address, 'online' => $this->online, 'refresh' => $this->refresh, 'port' => $this->port, 's_user' => $this->s_user, 'name' => $this->name, 'method' => $this->method, 'info' => $this->info);
	}
