...
function mcraftQuery_SE($host, $port = 25565, $timeout = 2) {
	...
	if ($data[1] === "\xA7" && $data[2] === "\x31") {
		...
		return Array('hostname' => $data[3], 'numpl' => (int)$data[4], 'maxplayers' => (int)$data[5], 'protocol' => (int)$data[1], 'version' => $data[2]);
	}

	$data = explode("\xA7", $data);

	return Array('hostname' => substr($data[0], 0, -1), 'numpl' => isset($data[1]) ? (int)$data[1] : 0, 'maxplayers' => isset($data[2]) ? (int)$data[2] : 0);
}
