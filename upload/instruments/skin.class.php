...
class skinGenerator2D {
	...
	private static function ratio($file, $baze = 64, $prop = 2) {
		...
		if (round($input_size[0] / $input_size[1], 2) != round($prop, 2))
			return false; else if ($input_size[0] < $baze)
			return false;
	...
	public static function savePreview($way_save, $way_skin, $way_cloak = false, $side = false, $size = 224) {
		...
		if ($side)
			$new_skin = half_x_image($new_skin, $side);
	...
	public static function isValidCloak($way_cloak) {
		...
		$ratio1 = self::ratio($way_cloak, self::CLOAK_BASE, self::CLOAK_PROP);
		$ratio2 = self::ratio($way_cloak, self::SKIN_BASE, self::SKIN_PROP);
