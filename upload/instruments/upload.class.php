...
Class File extends View {
	...
	public function Create($post_name, $user_id, $id_word = false, $id_rewrite = false) {
		global $db;
		$user_id = (int)$user_id;
		if (!POSTGood($post_name, $this->formats))
			return 1;
	...
	public function getInfo() {
		...
		return array('id' => $this->id, 'id_word' => $this->id_word, 'user_id' => $this->user_id, 'name' => $this->name, 'size' => $this->size, 'downloads' => $this->downloads, 'hash' => $this->hash);
	}
...
Class FileManager extends View {
	...
	public function ShowFilesByUser($list = 1, $user_id = false) {
		...
		if (is_numeric($user_id))
			$sql_part = " WHERE `user_id`='".$db->safe((int)$user_id)."'";
