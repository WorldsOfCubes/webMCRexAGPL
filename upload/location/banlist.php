...
if (!empty($user) and $user->lvl() > 0) {
	if ($query = $db->execute("SELECT * FROM `banlist` WHERE `name`='".$user->name()."'")) {
		$tmp_item = $db->fetch_assoc($query, 0);
...
while ($tmp_item = $db->fetch_assoc($query, 0)) {
	$tmp_item['name'] = TextBase::HTMLDestruct($tmp_item['name']);
	$tmp_item['reason'] = TextBase::HTMLDestruct($tmp_item['reason']);
	$tmp_item['admin'] = TextBase::HTMLDestruct($tmp_item['admin']);
	...
	switch ($tmp_item['type']) {
		case 0:
			...
			break;
		case 1:
			$tmp_item['type'] = 'Бан по IP';
			break;
		case 2:
			$tmp_item['type'] = 'Предупреждение';
			break;
		case 3:
			$tmp_item['type'] = 'Кик';
			break;
		case 4:
			...
			break;
		case 5:
			$tmp_item['type'] = 'Разбан';
			break;
		case 6:
			$tmp_item['type'] = 'Тюрьма';
			break;
		case 9:
			$tmp_item['type'] = 'Перманентный бан';
			break;

		default:
			$tmp_item['type'] = 'Неизвестно';
			break;
	}
...
$result = $db->execute("SELECT COUNT(*) FROM `banlist`");
